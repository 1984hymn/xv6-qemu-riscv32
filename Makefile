...
# TOOLPREFIX = /opt/riscv/bin/riscv32-unknown-elf-
...
#how to use mkfs tool? is mkfs fs.img files...\n It means the fs.img is the name of output, which has a same name as makefile target "fs.img" 
...
xv6_result: $K/kernel fs.img
	echo This two files are ready, they are based on riscv32
...
QEMUOPTS += -d guest_errors,in_asm -D qemulog.txt	#for debug, can record an appropriate ASM running process. If not set breakpoint in ISR/trap handler, it will skin it and won't record it.
#QEMUOPTS += -S -gdb tcp::1234	#-S will block qemu until the gdb engage
